...
bool eval_perm(char* path, argument_t* argument, struct stat* buf) {
	...
	perm_char[0] = ((buf->st_mode & S_IRUSR) ? 'r' : 'n');
	perm_char[1] = ((buf->st_mode & S_IWUSR) ? 'w' : 'n');
	perm_char[2] = ((buf->st_mode & S_IXUSR) ? 'x' : 'n');
	perm_char[3] = ((buf->st_mode & S_IRGRP) ? 'r' : 'n');
	perm_char[4] = ((buf->st_mode & S_IWGRP) ? 'w' : 'n');
	perm_char[5] = ((buf->st_mode & S_IXGRP) ? 'x' : 'n');
	perm_char[6] = ((buf->st_mode & S_IROTH) ? 'r' : 'n');
	perm_char[7] = ((buf->st_mode & S_IWOTH) ? 'w' : 'n');
